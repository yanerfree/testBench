...
| 19 | **环境配置页面重设计**：全局变量 Tab + 环境变量 Tab，Apifox 风格两层体系（已被 #58 取代） | John(PRD补充) + Amelia(实现) | P0 | ✅ 已被 #58 取代 |
| 20 | **环境变量优先级机制**：环境变量 > 全局变量 > 脚本配置，后端执行引擎合并后注入 os.environ（已被 #58 取代） | Amelia(后端) | P0 | ✅ 已被 #58 取代 |
...
### PRD v3.4 重大变更（2026-04-15）

> **来源：** Dreamer 提出，经讨论确认后选择方案 A（迭代模型）

| 序号 | 变更内容 | 责任人 | 优先级 | 状态 |
|------|---------|--------|--------|------|
| 57 | **项目引入迭代（Iteration）二级结构**：项目不再直接绑定分支，新增 `iterations` 表。用例、计划、报告、目录均挂在迭代下。不同迭代的数据完全隔离，解决多版本项目膨胀问题。 | John(PRD) + Amelia(全栈) + Sally(前端) | P0 | ✅ 已更新 PRD+架构 |
| 58 | **全局变量 + 环境变量两层体系（参考 Apifox）**：新增 `global_variables` 表存储跨环境通用变量；`environments` 表移除 `base_url` 独立字段改为环境变量 key；优先级：环境变量 > 全局变量 > 脚本配置。 | John(PRD) + Amelia(后端) + Sally(前端) | P0 | ✅ 已更新 PRD+架构 |
| 59 | **数据库 Schema 调整**：`projects` 表移除 `branch`/`json_file_path`/`last_sync_at`/`last_commit_sha` 字段移至 `iterations`；`cases`/`plans`/`case_folders`/`audit_logs` 的 `project_id` 改为 `iteration_id`；新增 `global_variables` 表 | Amelia(后端) | P0 | ✅ 已更新架构 |
| 60 | **API 端点调整**：用例/计划/目录的父资源从 `/api/projects/{id}/` 改为 `/api/iterations/{id}/`；新增迭代 CRUD + 归档/激活 API；新增全局变量 API；新增环境复制和合并变量预览 API | Amelia(后端) | P0 | ✅ 已更新架构 |
| 61 | **前端页面调整**：项目进入后新增迭代选择/切换层；环境配置页面新增全局变量 Tab；所有涉及用例/计划/报告的页面需在迭代上下文中工作 | Sally(前端) + Amelia(前端) | P0 | ⬜ 待实现 |

#### 变更背景说明

- **迭代模型原因：** 原设计项目绑定单分支，同一产品的不同版本（如 v1.0 在 `release/1.0`，v2.0 在 `release/2.0`）需创建多个项目，导致项目膨胀且用例/报告数据分散。不同分支的脚本和用例需各自保留，切换分支不能丢失原有数据。
- **环境变量原因：** 原设计只有一层环境变量，公共参数（如通用 token、超时配置）需在每个环境中重复配置。参考 Apifox 拆分为全局变量（跨环境）+ 环境变量（按环境区分），更合理。

---

*生成于 2026-04-15 · 来源：UX Step 3-8 + 架构 Step 1-5 + Party Mode 评审 + PRD v3.4 迭代模型+环境变量重构*
