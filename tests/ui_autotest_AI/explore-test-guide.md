...
> 版本：v2.0 · 更新：2026-04-16

---

## 一、技能简介

`explore-test` 是一个面向 **Web 页面的一次性系统测试技能**，通过 `playwright-cli` 驱动 Chromium 浏览器，全自动完成功能、安全、性能、UI 四个维度的测试，并将结果输出为结构化 Markdown 报告。

### 1.1 核心能力

| 维度 | 覆盖内容 |
|------|---------|
| **功能测试** | 列表加载、搜索/过滤、分页、新建/编辑/删除、详情展开、UI 三状态（空/加载/错误）；增强项 E1–E7：表单深度验证、状态驱动操作、搜索边界、操作后跳转、列表排序、导入导出、跨页数据一致性 |
| **UI 测试** | 响应式断点检查（375px 移动端 + 1920px 宽屏），截图核查布局溢出/重叠/折叠 |
| **安全测试** | XSS 注入（script/img）、SQL 注入、Mass Assignment 越权字段、无认证 API 访问、IDOR 越权读写、CORS 通配符、安全响应头；RBAC 角色权限隔离（配置次要账号时自动触发） |
| **性能测试** | Core Web Vitals（FCP/LCP/CLS）、页面加载时序、核心接口三次采样均值、失败请求检测 |

### 1.2 两种测试模式

| | spec 模式 | explore 模式（默认） |
|--|-----------|-------------------|
| **适用场景** | 有需求文档 / 接口文档 / UI 设计稿 | 无任何文档，直接对页面探索 |
| **预期依据** | 文档中的功能描述与接口契约 | 元素语义推断 + 通用交互惯例 |
| **额外步骤** | Step 0：读文档，提取定制化测试用例 | 无，从 Step 1 直接开始 |
| **功能测试方式** | 逐条规格验证，对照文档预期行为 | 单遍扫描，识别即推断即测试 |
| **报告独有内容** | 需求符合度百分比 + 需求编号溯源 | 元素推断覆盖说明 |
...
```
         ...
         │      spec 模式         │     explore 模式      │
         │  对照文档逐条规格验证    │  单遍扫描              │
         │  执行→对比预期→记录     │  识别→推断→测试→记录   │
         ├────────────────────────┴─────────────────────┤
         │  UI 三状态（必做）                             │
         │  空状态 · 加载态 · 网络错误态                   │
         ├──────────────────────────────────────────────┤
         │  增强项 E1–E7（检测到对应功能则执行，否则跳过）  │
         │  E1 表单深度验证    E2 状态驱动操作可用性        │
         │  E3 搜索边界        E4 操作后跳转逻辑            │
         │  E5 列表排序        E6 导入 / 导出              │
         │  E7 跨页数据一致性                             │
         └───────────────────────┬──────────────────────┘
         ...
         └───────────────────────┬──────────────────────┘
                                 │
         ┌───────────────────────▼──────────────────────┐
         │     Step 4.5  RBAC 角色权限【有次要账号时执行】  │
         │  -s=lowperm 低权限会话                         │
         │  · UI 层按钮可见性  · API 层垂直越权  · URL 直访│
         └───────────────────────┬──────────────────────┘
                                 │
         ┌───────────────────────▼──────────────────────┐
         │                 Step 5  性能测试               │
         ...
         │              Step 7  写入 error.md            │
         ├──────────────────────────────────────────────┤
         │  一 基本信息   二 测试方法   三 功能清单         │
         │  四 性能结果   五 缺陷集合   六 风险评估         │
         └──────────────────────────────────────────────┘
```

### 1.4 输出报告结构

报告共 6 个章节，写入 `error.md`：

```
一、基本信息      ← 测试环境快照
二、测试方法      ← 功能/UI/安全/性能四个维度说明
三、功能清单      ← F/U/S/R/P 五类共 30 项，含通过/失败状态 + 关联缺陷编号
四、性能结果      ← 8 项指标（FCP/LCP/CLS + 加载时序 + 接口三次采样）
五、缺陷集合      ← 完整 bug 表格 + 按严重级别统计
六、风险评估      ← 整体风险等级 + 优先修复建议（spec 模式含需求符合度）
```
...
**方式 A：从 `.env` 文件读取凭据（推荐）**

在项目根目录创建或确认 `.env` 文件包含以下字段：

```env
APP_URL=https://your-app.example.com/
USERNAME=admin
PASSWORD=your_password
USERNAME_LIJH=lowperm_user    # 可选：低权限账号，配置后自动触发 RBAC 测试
PASSWORD_LIJH=lowperm_pass
```

**方式 B：直接在调用时提供凭据**

在调用指令中直接写明用户名和密码（见下方示例）。
...
#### 示例 1：无文档，explore 模式（最常用）

```
/explore-test
目标页面：https://aam-ai-dev.paraview.cn/console/audit/tasks
报告路径：./discover_test/
测试模式：explore
```

Claude 会自动从 `.env` 读取凭据，完成登录、测试、写报告。
...
报告路径：./discover_test/
测试模式：spec
文档路径：./docs/audit-task-requirements.md, ./docs/audit-api.md
```

Claude 会先读取两份文档，提取功能点、接口契约、字段规则，再执行规格验证。
...
```
/explore-test
目标页面：https://staging.example.com/admin/users
报告路径：./reports/
测试模式：explore
凭据：用户名=testadmin，密码=Test@123
```
...
/explore-test
目标页面：https://aam-ai-dev.paraview.cn/console/audit/tasks
报告路径：./discover_test/task/
测试模式：explore
```

```
/explore-test
目标页面：https://aam-ai-dev.paraview.cn/console/user/list
报告路径：./discover_test/user/
测试模式：explore
```
...
| 参数 | 必填 | 说明 | 示例 |
|------|------|------|------|
| `目标页面` | ✅ | 要测试页面的完整 URL | `https://example.com/admin/tasks` |
| `报告路径` | ✅ | `error.md` 的输出目录 | `./discover_test/` |
| `测试模式` | ✅ | `explore`（默认）或 `spec` | `explore` |
| `文档路径` | spec 必填 | 需求/接口/设计文档路径，支持多个 | `./docs/req.md, ./docs/api.md` |
| `凭据` | 可选 | 不填则自动读 `.env` | `用户名=admin，密码=xxx` |
...
| 类型 | 项目 | 执行时机 |
|------|------|---------|
| 功能 | UI 三状态检查（空/加载/错误） | Step 3 功能测试末尾 |
| UI | 响应式断点检查（375px + 1920px） | Step 3.5 |
| 安全 | Mass Assignment 越权字段测试 | Step 4 阶段一 |
| 安全 | IDOR 越权读写测试 | Step 4 阶段二 |
| 性能 | Core Web Vitals（FCP/LCP/CLS）+ 三次采样 | Step 5 |

**条件触发项**（满足条件时自动执行）：

| 项目 | 触发条件 |
|------|---------|
| RBAC 角色权限验证（Step 4.5） | `.env` 配置了 `USERNAME_LIJH` / `PASSWORD_LIJH` |

**按需增强项 E1–E7**（检测到对应页面元素则执行，否则跳过）：

| 编号 | 场景 | 触发条件 | 典型发现问题 |
|------|------|---------|------------|
| E1 | 表单深度验证 | 存在新建/编辑表单 | 格式无校验、字段无长度限制、重名不拦截、字段联动失效 |
| E2 | 状态驱动操作可用性 | 列表有状态列 | 非法状态下操作按钮可点击、服务端缺少状态保护 |
| E3 | 搜索边界 | 存在搜索框 | 特殊字符导致 500、多条件仅最后一个生效、空格返回空结果 |
| E4 | 操作后跳转与刷新 | 执行增删改后 | 列表不自动刷新、失败后表单被清空 |
| E5 | 列表排序 | 列表表头可点击 | 排序图标更新但数据未重排（假响应） |
| E6 | 导入/导出 | 存在导入/导出按钮 | 导出不限权、导入错误无行级报告、格式错误导致 500 |
| E7 | 跨页数据一致性 | 有仪表盘/统计面板 | 增删后仪表盘计数未同步 |
...
技能会在登录后自动 `goto` 目标页面，SSO 跳转通常不影响最终结果。若循环重定向可在调用时手动指定已登录的页面 URL。

**Q：安全测试会修改真实数据吗？**

技能遵循「数据还原原则」：XSS payload 注入后若保存成功，会立即改回原始值；删除操作仅对测试中临时创建的数据执行，不操作已有数据。Mass Assignment 和 IDOR 测试使用 `fetch()` 直接发请求，不经过表单，不会触发页面跳转。

**Q：RBAC 测试什么时候会自动执行？**

仅当 `.env` 中配置了 `USERNAME_LIJH` 和 `PASSWORD_LIJH` 时触发（Step 4.5）。未配置则跳过，报告中 R01/R02 标记为"➖ 不适用"。推荐在有多角色场景的系统中配置，能发现最高频的 P0 垂直越权漏洞。

**Q：响应式截图需要手动核查吗？**

是的。技能自动完成 375px 和 1920px 两个断点的截图（`ui-375.png` / `ui-1920.png`），但布局是否合理需要人工判断。截图保存在当前工作目录的 `.playwright-cli/` 下，完成测试后人工核对即可。

**Q：spec 模式支持哪些文档格式？**

支持任何可被 `Read` 工具读取的文本格式：`.md`、`.txt`、`.yaml`、`.json`。不支持 PDF/Word，需先转换为文本。

**Q：token 消耗大吗？**

explore 模式使用单遍扫描（一次 snapshot，边识别边测），不重复读取页面，token 效率较高。新增的 Step 3.5（响应式）和 Step 4.5（RBAC）各自只需 3–5 条命令，额外 token 消耗约 15–20%。增强项 E7（跨页一致性）需要额外导航，资源紧张时可跳过。

---

### 2.7 报告输出位置

测试完成后，报告写入 `<报告路径>/report.md`：

```
discover_test/
└── report.md          ← 完整测试报告（6 章节）
.playwright-cli/
├── ui-375.png        ← Step 3.5 移动端截图
├── ui-1920.png       ← Step 3.5 宽屏截图
├── ui-loading.png    ← Step 3 加载态截图
└── ui-error.png      ← Step 3 错误态截图
```
...
1. **仅用于授权测试环境**：不要对生产环境或未获授权的系统使用此技能
2. **安全测试有副作用**：XSS payload 写入会被立即还原，但仍会产生网络请求；IDOR/Mass Assignment 测试会发送 API 请求，请确保测试环境允许
3. **`.env` 请勿提交到 Git**：凭据文件（含次要账号）应加入 `.gitignore`
4. **spec 模式文档质量影响结果**：文档越完整（有字段规则、接口契约、枚举值），测试用例越精准
5. **RBAC 测试需要明确次要账号的权限边界**：配置前确认该账号应被拒绝访问哪些接口/页面，避免误报

---

## 四、完整 Skill 文件内容

> 以下为 `explore-test` 技能的完整定义文件，供查阅 / 二次定制使用。
> 文件位置：`.claude/skills/explore-test/SKILL.md`
