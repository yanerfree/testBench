---
name: tb-doc-generate
description: 自动操作任意 Web 系统截图 + AI 生成带截图的操作文档
version: 3
inputs:
  - system_url: 被测系统地址（必填）
  - username: 登录账号（必填）
  - password: 登录密码（必填）
  - title: 文档标题（必填）
  - doc_type: 文档类型（manual/demo/acceptance）
  - modules: 文档范围（可选，逗号分隔）
  - audience: 目标读者（可选）
  - output_dir: 输出目录（Claude Code 模式）
  - business_context: 业务背景（可选）
---

# 文档生成 Skill (v4)

通用 Web 系统文档生成器，不依赖特定 UI 框架。

## 两种使用模式

- **平台内生成**：在 testBench「文档管理」页填系统地址/账号 → 平台跑 Playwright 截图 + AI 写文档（下方 Step 1~5 即平台端流程）。
- **Claude Code 客户端模式**：外部项目连上 testBench MCP 后，调用 `tb_get_doc_spec(doc_type)` 取回「操作流程 + 格式模板 + 写作规则」，由 Claude Code 用自己的浏览器工具在**本地实操被测系统、截图、按模板贴图**，文档落盘到项目 `docs/`。平台不代跑，只发规范。两种模式共用下方同一套「格式模板」。

## Step 1 — 启动浏览器并登录

1. 启动 Chromium 无头浏览器（1400x900）
2. 打开 `system_url`，截图登录页
3. 通用登录：
   - 按优先级查找用户名输入框：`id*=user` → `name*=user` → `placeholder*=用户` → `type=text`
   - 查找密码框：`type=password`
   - 查找提交按钮：`type=submit` → `text=登录/Login/Sign in`
4. 等待登录完成，截图首页

## Step 2 — 发现导航并截图

1. 通用导航发现（按优先级）：
   - 语义化：`nav a` / `aside a`
   - ARIA：`[role="menuitem"]`
   - 常见框架：`.ant-menu-item` / `.el-menu-item`
   - 通用：`[class*="sidebar"] a` / `[class*="nav"] a`
2. 如果指定了 `modules`：
   - 匹配的菜单标记为 `isTarget=true`
   - 目标模块做**深度截图**
3. 逐个点击菜单项截图
4. 最多 20 张截图

## Step 3 — 深度截图（目标模块）

对 `isTarget` 模块执行：
1. **限定搜索范围**：只在主内容区域找按钮，排除侧边栏/导航
   - 优先选择器：`main button` → `[class*="content"] button` → `[class*="main"] button`
   - 兜底：全页面 `button`
2. **关键词匹配**：新增/创建/添加/新建/Add/Create/New
3. **过滤无关按钮**：
   - 检查按钮可见性（`is_visible()`）
   - 排除按钮文字包含其他模块关键词（AI/Mock/MCP/LLM 等），除非目标模块本身是这些
   - 跳过已点击过的同名按钮
4. 点击按钮，等待 1.5 秒
5. 检测弹窗/对话框/抽屉：
   - 通用选择器：`[role="dialog"]` / `[class*="modal"]` / `[class*="drawer"]` / `[class*="dialog"]`
6. 截图弹窗状态
7. 关闭弹窗：
   - 找关闭按钮：`[class*="close"]` / `text=取消/Cancel/关闭/Close`
   - 兜底：按 Escape
6. 每个目标模块最多深度截 3 张

## Step 4 — AI 写文档

1. 从本 SKILL.md 读取对应文档类型的格式模板
2. 组装 AI prompt：
   - 系统 prompt：格式模板 + 约束规则
   - 用户 prompt：标题/范围/读者/业务背景 + 截图列表（⭐标记目标）
3. 约束规则：
   - 只详细写 `modules` 范围内的功能
   - ⭐目标模块截图展开详细操作步骤
   - **必须引用每一张截图**，不遗漏
   - 其他截图作为辅助说明配图
   - 操作步骤具体到按钮名称、输入内容、预期结果

## Step 5 — 保存文档

1. **截图质量检查**：跳过空名称/None 的截图
2. **平台模式**：保存到 documents 表 + screenshots 目录
3. **Claude Code 模式**：保存到 output_dir
4. **导出支持**：HTML（图片 base64 内嵌）/ ZIP（md + images/）/ 复制 Markdown

## 格式模板

### 文档类型格式

#### 演示文档（demo）
```markdown
# 场景标题 — 一句话描述演示内容

演示什么功能，完成什么工作流。用 2-3 句话概括。

涉及模块：模块A、模块B
__演示耗时__：约 N 分钟

## 场景概述

用 2-3 段话介绍这个场景的背景和价值：
- 第一段：功能做什么，解决什么问题
- 第二段：本场景演示的完整流程是什么

## 前置条件

使用什么角色登录（角色 / 账号密码）
业务数据：需要什么前提数据

> **操作提示：** 文档中的名称均为示例值。如遇"名称已存在"提示，替换为任意不存在的名称即可，不影响后续操作。

## 操作步骤

### 步骤一：动作名称（如"管理员登录"）

1. 打开系统 URL，进入登录页面
2. 输入账号密码，点击【登录】
	- 预期：提示「登录成功」，跳转到首页

![](截图URL)

登录页面

### 步骤二：动作名称（如"创建用户"）

1. 点击左侧菜单【用户管理】
2. 点击【新建用户】按钮
3. 填写用户信息：
   - 用户名：demo_tester
   - 密码：Test1234
   - 角色：普通用户
4. 点击【确定】
	- 预期：提示「创建成功」，用户出现在列表中

![](截图URL)

用户管理列表

![](截图URL)

新建用户弹窗

### 步骤三：...

（每个步骤都要有：编号操作 + 预期结果 + 截图 + 截图说明）

__演示话术__：一句话总结整个演示的核心价值。
```

#### 操作手册（manual）
```markdown
# 产品名称操作手册-模块篇

**公司/团队名称**
**日期**

---

# 1. 简介

## 1.1 模块概述

用 2-3 段话介绍模块的核心能力：
- 第一段：模块做什么，提供什么能力
- 第二段：支持哪些主要功能，适用什么场景
- 第三段：对团队/企业的价值

## 1.2 功能介绍

本模块提供以下核心功能：

- **功能A：** 用 2-3 句话详细描述这个功能的作用、支持的操作、适用场景
- **功能B：** 用 2-3 句话详细描述
- **功能C：** 用 2-3 句话详细描述

## 1.3 术语定义

| 术语和缩写 | 解释 |
|---|---|
| 术语A（English） | 详细解释，不少于一句话 |
| 术语B（English） | 详细解释 |

---

# 2. 操作指南

## 2.1 登录与权限说明

**登录操作：**

- 访问平台登录页面，输入用户名和密码进行身份认证
- 认证通过后进入平台主界面

**权限要求：**

本模块菜单仅对具有相应权限角色的用户可见。若您无法看到入口，请联系系统管理员分配权限。

> **提示：** 具备相应权限角色的用户，方可进行管理操作。

![](截图URL)

*图：登录页面*

## 2.2 功能A

### 2.2.1 子功能（如"添加记录"）

**适用场景：** 您需要做什么操作时使用。用一句话描述使用场景。

**前置条件：** 已完成什么前提操作（如已获取某配置、已登录等）。

**操作步骤：**

1. 进入【菜单名】页面，点击【Tab/按钮名】
2. 点击"操作按钮"
3. 填写表单信息：字段A、字段B、字段C
4. 点击"确定"
5. 预期：提示"创建成功"，记录出现在列表中

![](截图URL)

*图：操作说明*

![](截图URL)

*图：结果列表*

### 2.2.2 子功能（如"查看详情"）

**适用场景：** ...

**操作步骤：**

1. 在列表中找到目标记录
2. 点击操作列的"详情"按钮
3. 预期：弹出详情窗口

> **说明：** 补充说明信息，如字段含义、注意事项等。

### 2.2.3 子功能（如"配置选项"）

**适用场景：** ...

**操作步骤：**
...

| 配置项 | 说明 |
|---|---|
| 选项A | 选项A的作用和配置方法 |
| 选项B | 选项B的作用和配置方法 |

![](截图URL)

*图：配置界面*

> **注意：** 重要的提示信息用引用块标注。

## 2.3 功能B

### 2.3.1 ...

（每个功能都按 2.2 的结构展开：适用场景 + 前置条件 + 操作步骤 + 截图 + 图说）
```

#### 验收文档（acceptance）
```markdown
# 项目名称 验收文档

## 验收标准

| # | 验收项 | 验证方法 | 预期结果 | 通过标准 |
|---|--------|---------|---------|---------|
| 1 | 功能A | 操作步骤 | 成功 | 正常响应 |
| 2 | 功能B | 操作步骤 | 成功 | 数据一致 |

## 详细验证步骤

### 验收项 1：功能A

**操作步骤：**
1. 进入页面
2. 执行操作
3. 检查结果

![](截图URL)

结果：✅ 通过

### 验收项 2：功能B
...
```

## 适配说明

本 Skill 通过通用选择器适配任意 Web 系统：
- 不依赖特定 UI 框架（Ant Design / Element UI / Material 等）
- 导航发现按优先级尝试多种选择器
- 登录检测支持中英文按钮文字
- 弹窗检测基于通用 CSS 类名和 ARIA role
- 如果某个系统的导航结构特殊，可以在 business_context 中补充说明
