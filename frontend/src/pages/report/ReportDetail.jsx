...
const methodColors = { GET: '#6ecf96', POST: '#7c8cf8', PUT: '#f5b87a', DELETE: '#f08a8e' }
  ...
  return (
    <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <Tag style={{ background: '#f6ffed', color: '#6ecf96', border: 'none', fontWeight: 600 }}>已完成</Tag>
            </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        ...
        {/* 指标卡片 */}
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gridTemplateRows: '1fr 1fr', gap: 6 }}>
          ...
          {[
            ...
            <Card key={i} styles={{ body: { padding: '14px 12px', textAlign: 'center' } }}>
              <div style={{ fontSize: 20, fontWeight: 600, color: item.danger ? '#f08a8e' : '#1d2129', lineHeight: 1.2 }}>{item.value}</div>
      ...
      {/* 筛选栏 */}
      <Card styles={{ body: { padding: '10px 20px' } }} style={{ marginBottom: 8 }}>
        ...
        <div key={mod.moduleId} style={{ marginBottom: 8 }}>
          ...
          >
            ...
            <Space size={16}>
              ...
              {mod.error > 0 && <span style={{ fontSize: 13, color: '#f5b87a', fontWeight: 600 }}>{mod.error} 错误</span>}
              {mod.flaky > 0 && <span style={{ fontSize: 13, color: '#f0d86e' }}>{mod.flaky} Flaky</span>}
                ...
                <div key={scn.scenarioId} style={{ marginBottom: 4 }}>
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <Tag style={{ background: scn.executionType==='automated'?'#e6f4ff':'#fff7e6', color: scn.executionType==='automated'?'#7c8cf8':'#f5b87a', border: 'none', fontSize: 11 }}>
                  ...
                  {(scn.errorSummary || scn.remark) && (
                    <div style={{ marginLeft: 36, padding: '4px 0', fontSize: 12, color: scn.errorSummary ? '#f08a8e' : '#86909c' }}>
                    ...
                    <div style={{ marginLeft: 36, marginTop: 2, borderRadius: 8, overflow: 'hidden', border: '1px solid #f2f3f5' }}>
                      {steps[scn.scenarioId].map((step, idx) => (
                        ...
                        >
                          ...
                          <Tag style={{ background: step.statusCode===200?'#f6ffed':'#fff2f0', color: step.statusCode===200?'#6ecf96':'#f08a8e', border: 'none', fontSize: 11 }}>{step.statusCode}</Tag>
        ...
        {selStep && stepDetail && (
          <>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 20, padding: '10px 14px', background: '#fafbfc', borderRadius: 10 }}>
              ...
              <Tag style={{ background: selStep.statusCode===200?'#f6ffed':'#fff2f0', color: selStep.statusCode===200?'#6ecf96':'#f08a8e', border: 'none' }}>{selStep.statusCode}</Tag>
              ...
              { key: 'res', label: 'Response', children: (
                <div>
                  <div style={{ marginBottom: 12, display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    <Tag style={{ background: stepDetail.response.statusCode===200?'#f6ffed':'#fff2f0', color: stepDetail.response.statusCode===200?'#6ecf96':'#f08a8e', border: 'none' }}>{stepDetail.response.statusCode}</Tag>
                  </div>
                  <h4 style={{ fontSize: 13, color: '#86909c', marginBottom: 8 }}>Headers</h4>
